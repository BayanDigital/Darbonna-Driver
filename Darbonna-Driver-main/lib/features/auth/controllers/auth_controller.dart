...
class AuthController extends GetxController implements GetxService {
      ...
      if (response.body['data']['is_phone_verified'] == 0) {
        ...
        if (isPhoneNotVerified) {
          if (Get.find<SplashController>().config?.isFirebaseOtpVerification ??
            ...
            snackBarWidget('sms_gateway_not_integrate'.tr);
          }
        }
      }
    } else {
  ...
  Future<void> permanentDelete() async {
    ...
    if (response!.statusCode == 200) {
      ...
      Get.back();
      Get.offAll(() => const SignInScreen());
      snackBarWidget('successfully_delete_account'.tr, isError: false);
    } else {
      logging = false;
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> register(String code, SignUpBody signUpBody) async {
    ...
    if (response!.statusCode == 200) {
      ...
      if (configModel?.verification ?? false) {
        ...
        snackBarWidget('registration_completed_successfully'.tr,
            isError: false);
        Get.offAll(() => const SignInScreen());
      }
  ...
  Future<Response> sendOtp(
    ...
    if (response!.statusCode == 200) {
      _isOtpSending = false;
      snackBarWidget('otp_sent_successfully'.tr, isError: false);
    } else {
      _isOtpSending = false;
      ApiChecker.checkApi(response);
    }
      ...
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationFailed: (FirebaseAuthException e) {
          ...
          if (e.code == 'invalid-phone-number') {
            snackBarWidget('please_submit_a_valid_phone_number'.tr);
          } else {
            snackBarWidget(e.message?.replaceAll('_', ' ') ?? '');
          }
        },
  ...
  bool isOldPasswordCorrect(String oldPassword) {
    String savedPassword = '123456789';
