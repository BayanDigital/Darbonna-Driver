...
class RideController extends GetxController implements GetxService {
  ...
  Future<Response> bidding(String tripId, String amount) async {
    ...
    if (response.statusCode == 200) {
      Get.back();
      isLoading = false;
      snackBarWidget('bid_submitted_successfully'.tr, isError: false);
  ...
  Future<Response> tripAcceptOrRejected(
      ...
      {bool showSuccess = true, int index = 0}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (action == 'rejected') {
        ...
        snackBarWidget('trip_is_rejected'.tr, isError: false);
      } else {
        if (type == 'parcel') {
          totalParcelCount++;
        ...
        if (showSuccess) {
          snackBarWidget('trip_is_accepted'.tr, isError: false);
        }
  ...
  Future<Response> matchOtp(String tripId, String otp) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (otp.isEmpty) {
        snackBarWidget('trip_started'.tr, isError: false);
      } else {
        snackBarWidget('otp_verified_successfully'.tr, isError: false);
      }
  ...
  Future<Response> tripStatusUpdate(
    ...
    if (response.statusCode == 200) {
      ...
      snackBarWidget(message.tr, isError: false);
      isLoading = false;
    } else {
      isLoading = false;
      ApiChecker.checkApi(response);
  ...
  Future<Response> waitingForCustomer(
    ...
    if (response.statusCode == 200) {
      getRideDetails(tripId);
      isLoading = false;
      snackBarWidget('trip_status_updated_successfully'.tr, isError: false);
    } else {
      isLoading = false;
      ApiChecker.checkApi(response);
    }
