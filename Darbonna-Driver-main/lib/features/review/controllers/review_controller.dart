...
class ReviewController extends GetxController implements GetxService {
    ...
    if (response.statusCode == 200) {
      ...
      if (_reviewTypeIndex == 0) {
        if (reviewModel!.data![index].isSaved!) {
          snackBarWidget('review_saved_successfully'.tr, isError: false);
        } else {
          snackBarWidget('review_removed_successfully_from_saved_list'.tr,
              isError: false);
        }
      } else {
        snackBarWidget('review_removed_successfully_from_saved_list'.tr,
  ...
  Future<Response> submitReview(String id, int ratting, String comment) async {
    ...
    update();
    Response response =
        await reviewServiceInterface.submitReview(id, ratting, comment);
    if (response.statusCode == 200) {
      Get.back();
      snackBarWidget('review_submitted_successfully'.tr, isError: false);
