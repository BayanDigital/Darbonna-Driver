...
class HelpAndSupportController extends GetxController implements GetxService {
      ...
      if (_pickedImageFiles != null) {
        for (int i = 0; i < _pickedImageFiles!.length; i++) {
          _selectedImageList.add(
            MultipartBody('files[$i]', _pickedImageFiles![i]),
          );
        }
      }
  ...
  Future<Response> getConversation(String channelId, int offset) async {
    isLoading = true;
    Response response = await helpAndSupportServiceInterface.getConversation(
      channelId,
      offset,
    );
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        messageModel!.totalSize = MessageModel.fromJson(
          response.body,
        ).totalSize;
  ...
  void downloadFile({required String url, required String fileName}) async {
    ...
    if (GetPlatform.isIOS) {
      ...
      if (apiResponse.statusCode == 200) {
        ...
        if (fileExists) {
          ScaffoldMessenger.of(
            Get.context!,
          ).showSnackBar(SnackBar(content: Text('file_already_downloaded'.tr)));
          await OpenFile.open(filePathName);
        } else {
          apiResponse.listen(
            (d) => downloadData.addAll(d),
            onDone: () {
              savedFile.writeAsBytes(downloadData);
            },
          );
          snackBarWidget('downloaded_successfully'.tr, isError: false);
        }
      } else {
        snackBarWidget('download_failed'.tr, isError: true);
      }
    } else {
      String? task;
      ...
      if (fileExists) {
        snackBarWidget('file_already_downloaded'.tr);
        await OpenFile.open(filePathName);
      } else {
